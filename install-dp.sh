#!/bin/bash
# ═══════════════════════════════════════════════════════════════
#  NOOBS ZIVPN UDP — Installer (Fixed v2)
# ═══════════════════════════════════════════════════════════════
...
hash_input() {
  if command -v sha256sum &>/dev/null; then
    ...
    echo -e "${R}No SHA-256 utility found.${NC}"
    exit 1
  fi
}
...
[ "$authorized" -eq 1 ] || { echo -e "${R}Too many failed attempts.${NC}"; exit 1; }
...
apt-get install -y -qq curl wget jq openssl python3 iptables iptables-persistent \
  netfilter-persistent bc vnstat conntrack-tools \
  gcc make pkg-config libnetfilter-queue-dev libnetfilter-conntrack-dev \
  libmnl-dev libnfnetlink-dev || warn "Some packages may not have installed"
...
wget -q --timeout=30 \
  ...
  -O "$BIN_PATH" || fail "Binary download failed"
...
wget -q --timeout=30 "$REPO_RAW/panel/zivudp.sh" -O "$PANEL_PATH" || warn "Panel download failed"
if [ -s "$PANEL_PATH" ]; then
  sed -i 's/\r$//' "$PANEL_PATH"
  chmod +x "$PANEL_PATH"
  ok "Terminal panel installed"
fi

wget -q --timeout=30 "$REPO_RAW/panel/webpanel.py" -O "$WEBPANEL_PATH" || warn "Web panel download failed"
if [ -s "$WEBPANEL_PATH" ]; then
  sed -i 's/\r$//' "$WEBPANEL_PATH"
  chmod +x "$WEBPANEL_PATH"
  ok "Web panel downloaded"
fi

step 9 "Compiling accounting daemon (zivacctd)..."
...
void *nfq_thread(void *arg) {
    ...
    qh = nfq_create_queue(h, NFQUEUE_NUM, &packet_callback, NULL);
...
static int conntrack_cb(enum nf_conntrack_msg_type type,
    ...
    // Fixed: These functions return the value directly, not via pointer
    src_ip = nfct_get_attr_u32(ct, ATTR_ORIG_IPV4_SRC);
    src_port = nfct_get_attr_u16(ct, ATTR_ORIG_PORT_SRC);
    dst_ip = nfct_get_attr_u32(ct, ATTR_ORIG_IPV4_DST);
    ...
    orig_bytes = nfct_get_attr_u64(ct, ATTR_ORIG_COUNTER_BYTES);
    reply_bytes = nfct_get_attr_u64(ct, ATTR_REPL_COUNTER_BYTES);
...
if systemctl is-active --quiet zivacct; then
  ...
  warn "Accounting daemon failed to start"
fi
