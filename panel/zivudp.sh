...
# Ensure required tools are installed. conntrack-tools is required for
# accurate per-password device/connection detection (plain `ss` cannot see
# individual UDP peers, which is why device counts used to always read 0) --
# everything else here was already an implicit dependency, just never
# verified up front. Missing packages are batched into a single apt-get
# call instead of one process per tool.
ensure_dependencies() {
  local -A pkg_for=(
    [jq]=jq
    [python3]=python3
  ...
  # conntrack needs the kernel's connection-tracking module; load it if it
  # isn't already (most cloud kernels have it built in or auto-loaded via
  # iptables NAT use, so this is a harmless no-op on most systems).
...
if '$pass' not in meta:
    expiry = None
    if $valid_days > 0:
        # Pure UTC epoch arithmetic -- time.time() is always seconds since
        # the Unix epoch in UTC regardless of the server's local timezone,
        # so no external time service or local-time formatting is needed.
...
clear_passwords() {
  ...
  echo '{}' > "$META_FILE"
  # `iptables -L` prints chain headers as "Chain ZIV_USER_xxx (...)", not a
  # line starting with the chain name -- the old `grep '^ZIV_USER_'` never
  # matched anything, so chains (and their INPUT rules) were never actually
  # removed. `iptables -S` prints "-N ZIV_USER_xxx" creation directives,
  # which we can match directly, and delete_iptables_chain (fixed above)
  # properly drops the INPUT references before flushing/deleting each one.
...
delete_iptables_chain() {
  local pass="$1"
  local chain="ZIV_USER_$pass"
  # The previous version ran `iptables -D INPUT -j "$chain"`, which never
  # matches because the actual rules also carry -s <ip> -p udp criteria --
  # iptables -D requires an exact match of ALL of a rule's criteria, not
  # just its target. That meant every removal silently failed and left a
  # stale per-IP rule behind forever. Scan by line number instead.
  local line
...
screen_add_user() {
  ...
  echo -e "  ${G}  ╚═══════════════════════════════════════════════╝${NC}"
  if { [ "$dev_limit" != "0" ] || [ "$data_gb" != "0" ] || [ "$valid_days" != "0" ]; } && ! webpanel_running; then
    echo -e "\n  ${Y}  ⚠  Device/data/expiry limits are only enforced while the Web Panel${NC}"
...
screen_trial_user() {
  ...
  # Stamp a real expiry into the metadata (not just a same-session sleep)
  # so that if the web panel is installed, its own monitor loop enforces
  # this reliably even after this terminal session ends -- a backgrounded
  # `sleep` job is not guaranteed to survive the parent shell exiting.
  python3 -c "
import json, time
META='$META_FILE'
meta = json.load(open(META))
...
screen_monitor() {
  ...
  if command -v conntrack &>/dev/null; then
    # conntrack sees the real client IP/port even for a connectionless UDP
    # server (and even through a DNAT'd/port-hopped public range), unlike
    # `ss`, which only ever shows the single listening socket. Parsed in
    # Python (not sed) so the original-tuple src is identified correctly
    # rather than by a greedy regex guess.
...
"
  ...
  # Retire any previously-installed zivmon monitor. webpanel.py now runs
  # its own internal monitor loop (conntrack-based device detection, real
  # byte-counter bandwidth accounting, fast expiry/device-limit
  # enforcement) inside the zivpanel process itself -- a separate zivmon
  # loop would just fight it (it used to flush/reset the same quota byte
  # counters every 60s, and its expiry check shelled out to a `zivudp
  # remove` subcommand that doesn't exist as a non-interactive command).
  ...
  sleep 2
  if webpanel_running; then
    echo -e "  ${G}✔ zivpanel is running — limits/expiry are now enforced automatically.${NC}"
  else
