...
(Device limit and data limit removed – only password + expiry)
"""
...
def _parse_endpoint(value):
    ...
    if value.startswith("[") and "]" in value:
        ...
        ip = value[1:close]
        rest = value[close + 1:]
        return ip, rest[1:] if rest.startswith(":") else ""
    if ":" in value:
...
def get_os_connections(real_port=None):
    """
    Return all currently visible UDP flows for the ZIVPN listen port.
    This is deliberately independent of configured/authenticated users.
    """
    ...
    # Preferred: kernel conntrack table.
    try:
        r = subprocess.run(
            ["conntrack", "-L", "-p", "udp", "-n"],
        ...
        for line in (r.stdout or "").splitlines():
            ...
            if real_port not in (orig_dport, reply_sport):
                continue

            if reply_sport == real_port:
                remote_ip = orig.get("src", "")
                remote_port = orig.get("sport", "")
            else:
                remote_ip = orig.get("dst", "")
                remote_port = orig.get("dport", "")

            key = (remote_ip, str(remote_port), "UDP")
            if key in seen:
                continue
            seen.add(key)
            ...
            connections.append({
                ...
                "local_port": str(real_port),
                "state": "UNREPLIED" if "UNREPLIED" in line.upper() else "ASSURED",
                "source": "conntrack",
            })
    ...
    if connections:
        return sorted(connections, key=lambda x: (x["ip"], x["port"]))

    # Fallback: kernel socket table via ss.
    try:
        r = subprocess.run(
            ["ss", "-Hanu"],
            capture_output=True, text=True, timeout=3
        ...
        for line in (r.stdout or "").splitlines():
            ...
            if len(parts) < 5:
                continue
            local_ip, local_port = _parse_endpoint(parts[4])
            peer = parts[5] if len(parts) > 5 else ""
            peer_ip, peer_port = _parse_endpoint(peer)

            if local_port != str(real_port):
                continue

            key = (peer_ip, str(peer_port), "UDP")
            if key in seen:
                continue
            seen.add(key)
...
def get_os_connection_count(real_port=None):
    return len(get_os_connections(real_port))


# Existing user/device mapping retained for the Users page.
def get_conntrack_client_ips(real_port):
    return {c["ip"] for c in get_os_connections(real_port) if c.get("ip")}
...
def build_active_device_map(ip_to_pw):
    if not ip_to_pw:
        return {}
    active_ips = get_conntrack_client_ips(get_listen_port_int())
    pw_to_ips = {}
...
# ========== IPTABLES HELPERS (only for blocking expired users) ==========
...
# ========== METADATA HELPERS (only expiry + created_at) ==========
def load_meta():
    try:
...
def init_meta_for_user(pw, validity_days=0):
    meta = load_meta()
    if pw in meta:
        return
    expiry = None
...
# ========== MONITOR LOOP (only expiry enforcement) ==========
...
def add_user(pw, validity_days=0):
    d = read_zivpn()
    ...
    if pw not in d["auth"]["config"]:
        ...
        write_zivpn(d)
        init_meta_for_user(pw, validity_days)
        return True
    return False
...
def get_status_snapshot():
    ...
    # Global connection count is OS-level and does not depend on user mapping.
    connection_count = get_os_connection_count(get_listen_port_int())
...
# ========== HTML (updated: no device/data limit fields, Clear Expired button) ==========
HTML = r"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<div id="app" style="display: none;">
  ...
  <div class="sidebar" id="sidebar">
    ...
    <div class="nav-item" onclick="showPage('connections')">
      <i>🔗</i> <span>Current Connections</span>
    </div>
  ...
  <div class="main-content" id="mainContent">
    ...
    <div id="page-users" class="page" style="display:none">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <button class="btn btn-danger btn-sm" onclick="clearExpiredUsers()">Clear Expired Users</button>
        </div>
        <div class="card-body table-responsive">
          <table id="usersTable">
            <thead><tr><th>#</th><th>Password</th><th>Devices</th><th>Expiry (days)</th><th>Action</th></tr></thead>
    ...
    <!-- Current Connections Page -->
    <div id="page-connections" class="page" style="display:none">
      <div class="card">
        <div class="card-header">
          <span class="card-title">🔗 Current Connections (OS-level)</span>
          <div style="display:flex; gap:0.5rem; align-items:center; flex-wrap:wrap">
            <span id="connectionsCount" style="font-size:0.8rem; color:#64748b">0 connections</span>
            <button class="btn btn-primary btn-sm" onclick="loadConnections()">⟳ Refresh</button>
          </div>
        </div>
        <div class="card-body table-responsive">
          <table id="connectionsTable" style="min-width:700px">
            <thead>
              <tr><th>#</th><th>Remote IP</th><th>Remote Port</th><th>Local Port</th><th>Protocol</th><th>State</th><th>Source</th></tr>
            </thead>
...
<div id="monitorModal" class="modal">
  <div class="modal-content">
    ...
    <div><strong>Active devices:</strong> <span id="monitorDevices">0</span></div>
...
async function loadConnections() {
  ...
  if (!Array.isArray(data.connections) || data.connections.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:#64748b">No active UDP connections</td></tr>';
    return;
  }

  tbody.innerHTML = data.connections.map((c, idx) => `
    <tr>
      <td>${idx + 1}</td>
      <td style="font-weight:500; color:#2c7da0">${escapeHtml(c.ip || '—')}</td>
      <td>${escapeHtml(String(c.port || '—'))}</td>
      <td>${escapeHtml(String(c.local_port || '—'))}</td>
      <td>${escapeHtml(c.protocol || 'UDP')}</td>
...
async function addUser() {
  ...
  const res = await apiCall('/api/user/add', 'POST', {
    password: pw, validity_days: days
  });
  if (res.ok) {
...
async function bulkAdd() {
  ...
  let added = 0;
  for (const pw of pws) {
    const res = await apiCall('/api/user/add', 'POST', { password: pw, validity_days:0 });
    if (res.ok) added++;
  }
...
async function clearExpiredUsers() {
  if (!confirm('Remove all expired users?')) return;
  const res = await apiCall('/api/user/clear_expired', 'POST');
  if (res.ok) {
    showToast(`${res.removed || 0} expired user(s) removed`, 'success');
    loadUsers();
    loadDashboard();
  } else {
    showToast('Failed to clear expired users', 'error');
  }
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            ...
            connections = get_os_connections(get_listen_port_int())
            self.send_json({"count": len(connections), "connections": connections})
        else:
    ...
    def do_POST(self):
        ...
        if path == "/api/user/add":
            ...
            if ok:
                svc_action("restart")
            self.send_json({"ok": ok})
        elif path == "/api/user/clear_expired":
            # Identify and purge expired users
            meta = load_meta()
            now = time.time()
            expired = [pw for pw, data in meta.items()
                       if data.get("expiry") is not None and data["expiry"] <= now]
            purge_expired_users(expired)
            self.send_json({"ok": True, "removed": len(expired)})
...
if __name__ == "__main__":
    ...
    print(f"NOOBS ZIVPN Web Panel (password + expiry only)  ►  http://{ip}:{port}")
    try:
