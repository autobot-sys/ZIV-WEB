...
def user_statistics():
    db = load_users_db().get("users",[])
    active = len([u for u in db if u.get("enabled",True)])
    expired = len(db)-active
    expiring = len([u for u in db if u.get("enabled",True)])
    return {"total":len(db),"active":active,"expired":expired,"expiring":expiring}
...
def get_logs(n=30):
    try:
        r = subprocess.run(
            ["journalctl", "-u", "zivpn", f"-n{n}", "--no-pager", "--output=short-iso"],
...
def get_status_snapshot():
    ...
    if cached is not None and (now - STATUS_CACHE["ts"]) < STATUS_CACHE_TTL:
        return cached
...
def get_logs_cached(n=30):
    ...
    if cached is not None and (now - LOGS_CACHE["ts"]) < LOGS_CACHE_TTL:
        return cached
    data = get_logs(n)
...
async function loadStatus(){
  ...
  document.getElementById('s-conns').textContent = d.connection_count ?? 0;
...
# Enhanced users.json metadata support scaffold added.
