...
Compatible with external accounting daemon (zivacctd)
"""

import json, sys, subprocess, hashlib, secrets, time, socket, re, threading, os, tempfile
...
# Monitor loop still enforces device limits and expiry, but no longer
# performs data accounting (that is done by zivacctd).
MONITOR_INTERVAL = 5  # seconds

_state_lock = Lock()
_status_cache    = {"ts": 0.0, "data": {}}   # password -> status dict
_device_ip_cache = {"ts": 0.0, "data": {}}   # password -> [ips]

_quota_ips_by_pw   = {}  # pw -> set(ip) attached to its quota chain via INPUT
_blocked_ips_by_pw = {}  # pw -> set(ip) dropped in ZIV_BLOCKED
_device_first_seen = {}  # pw -> {ip: first_seen_epoch}
...
def setup_iptables_quota(pw, limit_gb):
    """Create the quota chain with quota and DROP rules. Only called when
    the chain doesn't already exist."""
...
def save_meta(meta):
    # Atomic write to avoid clobbering the daemon's concurrent updates.
    dirname = os.path.dirname(META_FILE)
    fd, tmpfile = tempfile.mkstemp(prefix=".meta_", dir=dirname)
    try:
        with os.fdopen(fd, "w") as f:
            json.dump(meta, f, indent=2)
        os.replace(tmpfile, META_FILE)
    except Exception:
        try:
            os.unlink(tmpfile)
        except Exception:
...
# ========== ACCOUNTING DAEMON ==========
def acct_daemon_running():
    try:
        r = subprocess.run(["systemctl", "is-active", "zivacct"],
                            capture_output=True, text=True, timeout=2)
        return r.stdout.strip() == "active"
    except Exception:
        return False

def restart_acct_daemon():
    if acct_daemon_running():
        subprocess.run(["systemctl", "restart", "zivacct"], stderr=subprocess.DEVNULL)
...
def monitor_tick():
    ...
    for pw, data in meta.items():
        ...
        # Data usage is now updated by zivacctd, so we don't touch it here.
        # We only ensure the quota chain exists and is attached to allowed IPs.
        if data_limit > 0:
            if not chain_exists(f"ZIV_USER_{pw}"):
                setup_iptables_quota(pw, data_limit / 1024**3)
            sync_quota_attachment(pw, allowed_ips)

        used_bytes = data.get("data_used_bytes", 0)  # from daemon
        remaining_days = -1
        if expiry_ts:
...
if __name__ == "__main__":
    ...
    print(f"NOOBS ZIVPN Web Panel (with zivacctd integration)  ►  http://{ip}:{port}")
    try:
