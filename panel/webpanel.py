#!/usr/bin/env python3
"""
NOOBS ZIVPN UDP Web Panel — Professional UI (cPanel-like)
...
# ========== CONFIG & USER MANAGEMENT (unchanged) ==========
def load_panel_conf():
    try:
...
# ========== PROFESSIONAL HTML UI (cPanel-like) ==========
HTML = r"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes">
  <title>ZIVPN | Control Panel</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
async function loadUsers() {
  ...
  tbody.innerHTML = data.users.map((u, idx) => `
    <tr class="clickable-row" ondblclick="showMonitor('${u.password}')">
      <td>${idx+1}</td>
...
if __name__ == "__main__":
    ...
    print(f"NOOBS ZIVPN Web Panel (professional UI)  ►  http://{ip}:{port}")
    try:
